...
def sttd_reshape(alamouti_txSignal: tf.Tensor, UeAxis=-3, SymbolAxis=-2) -> tf.Tensor:
    ...
    Parameters:
        ...
        SymAxis: the axis that indexes the OFDM symbol number of the transmit signal
    Returns:
    ...
    for i_UE in range(numTxUe):
        ...
        to_be_stacked.append(tf.pad(alamouti_txSignal[*slicing],padding)) # Seperate out UE i_UE's data symbols and pad before and after it with zeros
        pass
    if numTxUe>0:
        txSignal_reshaped = tf.concat(to_be_stacked, axis=-3)
    elif numTxUe==0:
        txSignal_reshaped = tf.reshape(alamouti_txSignal, (*signal_shape[:-2],signal_shape[-3]*signal_shape[-2],signal_shape[-1]))
...
def sttd_reshape_v2(alamouti_txSignal:tf.Tensor, SymbolAxis=-2) -> tf.Tensor:
    ...
    txSig_reshaped = tf.reshape(alamouti_txSignal,(*(signal_shape[:-2]),numSyms//2,2,2))  # [..., numSyms/2 , 2 , 2]
