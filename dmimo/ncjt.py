...
def adjust_channel(h:tf.Tensor, pathloss_dB:tf.Tensor, nAntTxList, nAntRxList, cfg:NcjtSimConfig):
    ...
    # Turning this rx_pwr_raw tensor into a (num_subframes, 1, num_rx_ant, 1, num_tx_ant, num_ofdm_symbols, 1)-shaped tensor
    tensor_list = []
    ...
    for i in range(num_tx):
        tensor_list.extend([rx_pwr_raw[:,:,:,i:i+1,:]-10*np.log10(nAntTxList[i]) for _ in range(nAntTxList[i])]) # Adjusting power according to number of transmit antennas
    rx_pwr = tf.concat(tensor_list, axis = 3)

    rx_pwr = tf.reshape(rx_pwr, (*h.shape[:-1],1)) # (num_subframes, 1, num_rx_ant, 1, num_tx_ant, num_ofdm_symbols, 1)
    h_adjusted = h * tf.cast(tf.sqrt(10**(rx_pwr/10)),h.dtype)
    h_adjusted = tf.transpose(h_adjusted[:,0,:,0,:,:,:], (0,4,3,1,2)) # (num_subframes, num_subcarriers, num_ofdm_symbols, total_rx_antennas, total_tx_antennas)
...
def ncjt_alamouti_dmimo_phase(bit_streams:tf.Tensor, channel:tf.Tensor, cfg:NcjtSimConfig, 
    ...
    :param tf.Tensor bit_streams: 
        ...
        each of the `M` transmitters. If there are no errors in the first phase, then all of the
    ...
    for i_tx in range(cfg.num_TxBs+cfg.num_TxUe):
        ...
        x = tf.concat([x for _ in range(nAnt//2)], axis=-1) # [num_subframes, num_subcarriers, num_ofdm_symbols, total_tx_antennas]
        x_list.append(x)
    ...
    ry = tf.reshape(ry, (*ry.shape[:-3],num_ofdm_symbols//2,2,total_rx_antennas)) # (num_subframes, num_subcarriers, num_ofdm_symbols/2 , 2, total_rx_antennas)    
    ...
    # but that isn't generally true. In any case, we are going to feed the average of two consecutive OFMD symbol 
    # channel to the STBC decoder.
    h_freq_ns3_averaged = (h_freq_ns3[...,::2,:,:] + h_freq_ns3[...,1::2,:,:])/2 # (num_subframes, num_subcarriers, num_ofdm_symbols/2, total_rx_antennas, total_tx_antennas)
    # Now we need to sum over the respective transmit atennas
    h_freq_ns3_averaged = tf.add_n([h_freq_ns3_averaged[...,i*2:i*2+2] for i in range(total_tx_antennas//2)]) # (num_subframes, num_subcarriers, num_ofdm_symbols/2, total_rx_antennas, 2)

    y_list = []
    gains_list = []
    for i in range(cfg.num_RxBs + cfg.num_RxUe):
        if i == cfg.num_RxBs-1: rx_antenna_slice = slice(i*cfg.nAntRxBs,(i+1)*cfg.nAntRxBs)
        else: rx_antenna_slice = slice(cfg.num_RxBs*cfg.nAntRxBs+(i-cfg.num_RxBs)*cfg.nAntRxUe,cfg.num_RxBs*cfg.nAntRxBs+(i-cfg.num_RxBs+1)*cfg.nAntRxUe)
        y, gains = alamouti_decode(ry_noisy[...,rx_antenna_slice], h_freq_ns3_averaged[...,rx_antenna_slice,:])  # assuming perfect CSI # (num_subframes, num_subcarriers, num_ofdm_symbols)
        y = y/tf.cast(gains,y.dtype) # y.shape = gains.shape = (num_subframes, num_subcarriers, num_ofdm_symbols)    
        if do_demapping: y = demapper([y,no]) # Turn into bits. Shape = (num_subframes, num_subcarriers, num_ofdm_symbols*cfg.num_bits_per_symbol_phase2)    
        y_list.append(y) 
...
def ncjt_sim_all_phases(cfg:NcjtSimConfig, h_dmimo:tf.Tensor = None, perfect_phase1 = True, perfect_phase3 = True):
    ...
    add_noise = AWGN()
    bit_stream = binary_source([cfg.num_subframes_phase1, cfg.num_subcarriers,  cfg.num_ofdm_symbols * cfg.num_bits_per_symbol_phase1])
    ...
    rx_bit_stream_dmimo_list, gains_list = ncjt_alamouti_dmimo_phase(bit_stream_dmimo,h_dmimo,cfg,mapper_dmimo,demapper_dmimo,add_noise,no, do_demapping=True)
    ...
    if cfg.perSC_SNR: # In case we have the exact gain of each subcarrier
        ...
        snrs = tf.stack(gains_list,axis=-1)/no
        snrs = tf.ones_like(snrs) * tf.reduce_mean(snrs,axis=(-2,-3),keepdims=True) # Averaging over OFDM symbols and subcarriers
    ...
    y_combined = hard_log_likelihood(tf.stack(symbol_streams_at_RxBs,axis=-1), snrs, k_constellation=cfg.num_bits_per_symbol_phase2) #  (cfg.num_subframes_phase2, num_ofdm_symbols)
    detected_bits = demapper_dmimo([y_combined, no])  # (cfg.num_subframes_phase2,num_subcarriers, num_ofdm_symbols * cfg.num_bits_per_symbol_phase2)
